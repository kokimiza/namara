...
* **Claudeには公開の権限を渡さない。** Claudeが動くジョブのトークンは読み取り専用で、許可するコマンドも問題作成に必要なもの（`content.sh`・`verify.sh`・各コンパイラ等）に限る。pushするのは、検証を通ったパッチだけを受け取る別ジョブである。予定外のファイルが変わっていれば何も公開しない
