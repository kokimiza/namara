...
write ジョブ（リポジトリへの読み取り権限のみ）
  ...
  4. script/verify.sh（ゲート）: ページの形式と、コンパイル・静的解析・実行結果を確認
  5. 変更がその日の日付ページと archive.html だけであることを確認し、差分をパッチにする
publish ジョブ（書き込み権限）
  6. 最新の main に日付ページのパッチを当て、archive.html の行は content.sh で入れ直す
  7. commit・push → Cloudflare Pages が自動デプロイ（§9 の手順5）
```
...
* **公開してよいかの判断はモデルの自己申告に頼らない。** Claudeにも検証は走らせるが、公開の条件になるのはワークフローの `script/verify.sh` ステップの結果だけである。検証用ソースの規約（ok/ng/bug/stdout）とツールの設定は `script/verify.sh` の冒頭と namara-daily Skill §4 にある。ページに載ったコードの各行が検証用ソースに含まれていることも確認するので、「検証したコード」と「掲載したコード」がずれない
* **Claudeには公開の権限を渡さない。** Claudeが動くジョブのトークンは読み取り専用で、入力はこのリポジトリ自身のファイルだけである。pushするのは、検証を通ったパッチだけを受け取る別ジョブである。Bashのコマンドは許可リストで絞らない——問題を書くには自分で書いたコードをコンパイルして実行する必要があり（`verify.sh`もそれを実行する）、コマンド名のリストでは実行されるコードの中身を制限できないうえ、必要なコマンドが拒否されるとClaudeが作業を途中でやめてしまう（初回の実行で実際に起きた）。予定外のファイルが変わっていれば何も公開しない
